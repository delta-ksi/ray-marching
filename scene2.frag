...
float sdf(vec3 pos)
{
    ...
    // Box frame
    {
        vec3 p = pos - vec3(0., 0., 1.);
        d = min(d, boxframeSdf(p, vec3(2), 0.1));
    }

    // Torus 1
    {
        vec3 p = pos - vec3(0, 0, 1);
        d = min(d, torusSdf(p.zxy, vec2(0.8, 0.1)));
    }

    // Torus 2
    {
        vec3 p = pos - vec3(0, 0, 1);
        d = min(d, torusSdf(p.yzx, vec2(0.8, 0.1)));
    }

    // Torus 3
    {
        vec3 p = pos - vec3(0, 0, 1);
        d = min(d, torusSdf(p.xyz, vec2(0.8, 0.1)));
    }
